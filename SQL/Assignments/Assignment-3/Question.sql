...
select ename as [Employee Name] from emp where (YEAR(GETDATE())-Year(hire_date))>30 and (YEAR(GETDATE())-Year(hire_date))<40

select count(*) as 'TotalEmployees' from emp where (YEAR(GETDATE())-Year(hire_date))>30 and (YEAR(GETDATE())-Year(hire_date))<40

--17. Retrieve the names of departments in ascending order and their employees in descending order. 
select d.dname as [Department Name],e.ename as [Employee Name] from dept d join emp e
